...
If you're using [yeoman](http://yeoman.org) to generate your AngularJS project you're probably making use of [grunt](http://grunt.org) to build everything. The default build configuration runs ````ngmin``` for pre-minification of your script files. That means you will have to stick to some conventions for writing controllers, services, filters etc. Take a look at ```ngmin```. This is what I ran into:
